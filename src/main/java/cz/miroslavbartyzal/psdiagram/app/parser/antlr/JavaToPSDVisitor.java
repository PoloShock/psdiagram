...
public class JavaToPSDVisitor extends Java8BaseVisitor<Flowchart<LayoutSegment, LayoutElement>> {
	...
	private boolean pairedComment = false;
	...
	public Flowchart<LayoutSegment, LayoutElement> visitComment(Java8Parser.CommentContext ctx){
		System.out.println("comment text: " + ctx.getText());
		System.out.println("comment parent: " + ctx.getParent().getParent().getText());
		...
		if (ctx.getChildCount() > 0) {
			for (ParseTree child : ctx.children) {
				...
				if (child instanceof SwitchBlockContext) {
					...
					lastElement = actualSegment.addSymbol(lastElement, EnumSymbol.SWITCH.getInstance(text), innerSegmentCount);
					cz.miroslavbartyzal.psdiagram.app.gui.symbolFunctionForms.Switch.generateValues(lastElement, text, new String[0]);
