...
//            // for editing convenience purposes (not including deletes -> just insertion):
//            "-5-",
//            "-55.",
//            "[",
//            "[]",
...
//            "a ! b",
//            "a!b",
//            "1. + 1.", // we are adding decimal point to the first operand
//            "a!(b=a)",
//            "[1, , 2]",
//            "[1,,2]",
//            "[,]",
//            "-*5" // replacing 1 by -1; example: 1*5 -> select 1 and type -> -*5 -> -1*5
//    );
...
//            addAll(validExamplesEditConv);
            ...
            add("([1, 2])[1]");
            add("(e.e()).e");
            add("[]"); // empty array; array inicialization
        }
    };
    ...
    public void validInputsTest()
    {
        for (String validExample : validExamples) {
            Assert.assertTrue("This input should be valid: " + validExample,
                    ValueFilter.isValid(validExample));
//            if (validExamplesEditConv.contains(validExample) || VariableFilterTest.validExamplesEditConv.contains(
//                    validExample) || NoArrayVariableFilterTest.validExamplesEditConv.contains(
//                            validExample)) {
//                Assert.assertFalse(
//                        "This input should not be valid for leaving as is: " + validExample,
//                        ValueFilter.isValid(validExample).canBeLeftAsIs);
//            } else {
//                Assert.assertTrue("This input should be valid for leaving as is: " + validExample,
//                        ValueFilter.isValid(validExample).canBeLeftAsIs);
//            }
//
//            if (!validExample.equals("1. + 1.")) { // "1. + 1." is valid only if the first decimal point is added after all the rest has been typed. Otherwise space after decimal point is rightfully not valid.
...
//                            ValueFilter.isValid(partOfValidExample).isValid);
//                }
//            }
        }
    }
    ...
    public void invalidInputsTest()
    {
        for (String invalidExample : invalidExamples) {
            Assert.assertFalse("This input should be invalid: " + invalidExample,
                    ValueFilter.isValid(invalidExample));
        }
    }
